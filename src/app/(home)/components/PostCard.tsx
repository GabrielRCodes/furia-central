...
export function PostCard({ post }: { post: Post }) {
  ...
  const getVerifiedColor = (social: SocialMedia) => {
    switch (social) {
      case 'twitter':
      ...
      <CardContent className="px-6 flex-grow flex flex-col">
        <div className="flex items-center mb-3">
          <Avatar className="h-10 w-10 mr-3">
            <AvatarImage src="https://res.cloudinary.com/dnuayiowd/image/upload/v1745531725/LOGO-MAIN_linrk0.png" alt={post.author} />
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <span className="ml-2 flex items-center">
                    <MdVerified className={`h-4 w-4 ${getVerifiedColor(post.socialMedia)}`} />
                  </span>
                </div>
                <PostTimeStamp relativeTime={post.timestamp} author={post.author} />
              </div>
      ...
      <CardFooter className="px-6 grid grid-cols-1 sm:grid-cols-2 gap-4 border-t">
        ...
        <Button variant="outline" size="default" asChild className="flex items-center justify-center w-full">
          <Link href={post.url} target="_blank" className="flex items-center justify-center">
            {getSocialIcon(post.socialMedia)}
